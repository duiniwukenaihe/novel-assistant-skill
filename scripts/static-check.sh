...
check_skill() {
  ...
  # oh-story / novel-assistant are generated single-directory bundles. Their internal modules are
  # byte-synced from source skills and covered by source-skill deep checks plus
  # test-oh-story-bundle.bats. Avoid re-scanning the huge generated copy here:
  # macOS grep/awk/bash regex tooling can abort on very long mixed-language lines.
  if [ "$skill_name" = "oh-story" ] || [ "$skill_name" = "novel-assistant" ]; then
    echo "  [PASS] generated bundle deep file-reference checks covered by source skills"
    if [ "$errors" -eq 0 ]; then
      PASS=$((PASS + 1))
  ...
  local agent_names=()
  if [ -d "$REPO_ROOT/src/internal-skills/story-setup/references/templates/agents" ]; then
    for f in "$REPO_ROOT/src/internal-skills/story-setup/references/templates/agents/"*.md; do
  ...
  while IFS= read -r -d '' src_file; do
    ...
    done < <(awk '{ n = split($0, parts, "`"); for (i = 2; i <= n; i += 2) if (parts[i] ~ /\.md/) print parts[i] }' "$src_file" 2>/dev/null | sort -u || true)
