...
SKILL_DIR="$REPO_ROOT/src/internal-skills/story-setup"
...
TMP_DIR="$(mktemp -d)"
RUNTIME_SCRIPTS=()
# Runtime anchors: workflow-runner.js, workflow-supervisor.js,
# workflow-session-heartbeat.js. The checked list remains manifest-driven.
while IFS= read -r script_name; do
  RUNTIME_SCRIPTS+=("$script_name")
done < <(node -e 'const m=require(process.argv[1]); for (const file of m.scriptFiles || []) console.log(file)' "$REPO_ROOT/config/novel-assistant-bundle-files.json")
...
copy_agent_refs() {
  local root="$1"
  mkdir -p "$root/.claude/agent-references"
  cp -R "$AGENT_REFS_DIR" "$root/.claude/agent-references/novel-assistant"
  cp -R "$AGENT_REFS_DIR" "$root/.claude/agent-references/legacy-story-setup"
}
...
copy_runtime_scripts() {
  ...
  for script_name in "${RUNTIME_SCRIPTS[@]}"; do
    cp "$REPO_ROOT/scripts/$script_name" "$root/scripts/$script_name"
    chmod +x "$root/scripts/$script_name"
  done
  rm -rf "$root/scripts/lib"
...
assert_file "$HOOKS_DIR/guard-outline-before-prose.sh"
assert_file "$HOOKS_DIR/prose-quality-gate.sh"
...
assert_grep '逐文件同步|managed manifest' "$SKILL_FILE" "SKILL.md must describe managed per-file hook deployment"
assert_grep '绝不递归删除目标目录' "$SKILL_FILE" "SKILL.md must forbid recursive hook target deletion"
...
assert_grep 'guard-outline-before-prose\.sh' "$SKILL_FILE" "SKILL.md must mention outline-before-prose guard"
assert_grep 'prose-quality-gate\.sh' "$SKILL_FILE" "SKILL.md must mention prose-quality-gate hook"
assert_grep 'prose-quality-gate\.sh' "$SETTINGS_FILE" "settings-hooks.json must register prose-quality-gate hook"
assert_grep 'safe-bash-guard\.js' "$SETTINGS_FILE" "settings-hooks.json must register safe Bash guard"
assert_file "$HOOKS_DIR/safe-bash-guard.js"
assert_grep 'Write\|Edit\|MultiEdit' "$SETTINGS_FILE" "PostToolUse prose gates must cover MultiEdit as well as Write/Edit"
echo "  OK TS1 hook dependency completeness"

# TS2 — Deployment checklist/manifest parseability
# Required literal runtime anchor: token-cost-ledger.js
...
assert_grep 'normalize-punctuation\.js' "$SKILL_FILE" "deployment manifest missing normalize-punctuation runtime script"
assert_grep 'output-pollution-check\.js' "$SKILL_FILE" "deployment manifest missing output-pollution-check runtime script"
assert_grep 'runtime-guard-validate\.js' "$SKILL_FILE" "deployment manifest missing runtime-guard-validate runtime script"
assert_grep 'token-cost-ledger\.js' "$SKILL_FILE" "deployment manifest missing token-cost-ledger runtime script"
assert_grep 'workflow-entry-guard\.js' "$SKILL_FILE" "deployment manifest missing workflow-entry-guard runtime script"
assert_grep 'workflow-runtime-supervisor\.js' "$SKILL_FILE" "deployment manifest missing workflow-runtime-supervisor runtime script"
assert_grep 'workflow-state-machine\.js' "$SKILL_FILE" "deployment manifest missing workflow-state-machine runtime script"
assert_grep 'context-assembler\.js' "$SKILL_FILE" "deployment manifest missing context-assembler runtime script"
assert_grep 'memory-recommender\.js' "$SKILL_FILE" "deployment manifest missing memory-recommender runtime script"
assert_grep 'chapter-commit\.js' "$SKILL_FILE" "deployment manifest missing chapter-commit runtime script"
assert_grep 'safe-text-search\.js' "$SKILL_FILE" "deployment manifest missing safe-text-search runtime script"
assert_grep 'novel-assistant-sync-runtime\.js' "$SKILL_FILE" "deployment manifest missing novel-assistant-sync-runtime runtime script"
assert_grep 'CLI 授权最小化协议' "$SKILL_FILE" "story-setup must document CLI authorization minimization"
assert_grep 'long-analyze-plan\.js' "$SKILL_FILE" "deployment manifest missing long-analyze-plan runtime script"
assert_grep 'stage2-summary-quality-check\.js' "$SKILL_FILE" "deployment manifest missing stage2-summary-quality-check runtime script"
assert_grep 'stage2-grounding-check\.js' "$SKILL_FILE" "deployment manifest missing stage2-grounding-check runtime script"
assert_grep 'write-failure-triage\.js' "$SKILL_FILE" "deployment manifest missing write-failure-triage runtime script"
assert_grep 'story-domain-profile\.js' "$SKILL_FILE" "deployment manifest missing story-domain-profile runtime script"
assert_grep 'chapter-volume-count\.js' "$SKILL_FILE" "deployment manifest missing chapter-volume-count runtime script"
assert_grep 'author-voice-profile\.js' "$SKILL_FILE" "deployment manifest missing author-voice-profile runtime script"
assert_grep '运行时脚本|runtime scripts|scripts/' "$SKILL_FILE" "SKILL.md must document runtime script deployment"
for script_name in "${RUNTIME_SCRIPTS[@]}"; do
  assert_file "$REPO_ROOT/scripts/$script_name"
done
runtime_root="$TMP_DIR/runtime-scripts"
mkdir -p "$runtime_root"
copy_runtime_scripts "$runtime_root"
for script_name in "${RUNTIME_SCRIPTS[@]}"; do
  assert_file "$runtime_root/scripts/$script_name"
  [ -x "$runtime_root/scripts/$script_name" ] || fail "runtime script not executable after deployment: $script_name"
done
assert_file "$runtime_root/scripts/lib/oh-story-artifacts.js"
assert_grep 'references_dir' "$SKILL_FILE" "sentinel references_dir must be documented"
assert_grep 'novel_assistant_bundle_id' "$SKILL_FILE" "sentinel novel_assistant_bundle_id must be documented"
assert_grep 'novel_assistant_source_commit' "$SKILL_FILE" "sentinel novel_assistant_source_commit must be documented"
...
stability_refs=(chapter-index.md chapter-contract.md plot-drift-control.md state-delta-ledger.md character-invariants.md revision-impact-analysis.md chapter-handoff-pack.md cross-chapter-continuity-audit.md longform-daily-stability-audit.md stability-repair-dispatcher.md stability-repair-loop.md stability-agent-dispatch-prompts.md error-codes.md user-style-learning.md)
...
while IFS= read -r ref; do
  ...
  assert_file "$refs_tmp/.claude/agent-references/novel-assistant/$ref"
  assert_file "$refs_tmp/.claude/agent-references/legacy-story-setup/$ref"
done < <(grep -RhoE 'novel-assistant/references/agent-references/[A-Za-z0-9_-]+\.md' \
  "$SKILL_DIR/references/templates/agents" "$AGENT_REFS_DIR" "$SKILL_DIR/references/templates/rules" 2>/dev/null \
  | sed 's|.*/||' | sort -u)
[ "$refs_seen" -gt 0 ] || fail "no novel-assistant agent-reference paths found in templates/rules"
for name in "${stability_refs[@]}"; do
  assert_file "$AGENT_REFS_DIR/$name"
  if [ "$name" = "error-codes.md" ]; then
    cmp -s "$REPO_ROOT/src/internal-skills/story-review/references/$name" "$AGENT_REFS_DIR/$name" || fail "deployed stability reference diverged from story-review source: $name"
  else
    cmp -s "$REPO_ROOT/src/internal-skills/story-long-write/references/$name" "$AGENT_REFS_DIR/$name" || fail "deployed stability reference diverged from story-long-write source: $name"
  fi
done
...
# TS4.1 — .active-book must only honor a valid first line inside the project.
# A blank/whitespace first line must not accidentally resolve the repository root;
# a valid second line is intentionally ignored so malformed files fall back to normal discovery.
active_book_root="$TMP_DIR/active-book-first-line"
mkdir -p "$active_book_root/fallback/追踪" "$active_book_root/selected/追踪"
setup_git_repo "$active_book_root"
copy_hooks "$active_book_root"

resolve_active_book() {
  local root="$1"
  (cd "$root" && CLAUDE_PROJECT_DIR="$root" bash -c 'source .claude/hooks/lib/common.sh; discover_active_book')
}

assert_active_book_path() {
  local label="$1"
  local expected="$2"
  local actual expected_real
  actual="$(resolve_active_book "$active_book_root")"
  expected_real="$(cd "$expected" && pwd -P)"
  [ "$actual" = "$expected_real" ] || fail "$label resolved [$actual], expected [$expected_real]"
}

printf '\nselected\n' > "$active_book_root/.active-book"
assert_active_book_path "blank .active-book first line" "$active_book_root/fallback"

printf '   \t  \nselected\n' > "$active_book_root/.active-book"
assert_active_book_path "whitespace-only .active-book first line" "$active_book_root/fallback"

printf 'missing-book\n' > "$active_book_root/.active-book"
assert_active_book_path "invalid .active-book target" "$active_book_root/fallback"

printf 'selected\nignored-second-line\n' > "$active_book_root/.active-book"
assert_active_book_path "valid .active-book first line" "$active_book_root/selected"

assert_grep 'split\("\\n"\)\[0\]\.trim\(\)' "$SKILL_DIR/references/opencode/plugin.ts" "OpenCode resolver must only read the active-book first line"
assert_grep 'resolved\.startsWith\(normalizedRoot \+ path\.sep\).*fs\.existsSync\(resolved\)' "$SKILL_DIR/references/opencode/plugin.ts" "OpenCode resolver must validate active-book target containment and existence"
echo "  OK TS4.1 active-book first-line resolver policy"

# TS4.2 — Agent restart confirmation marker
restart_root="$TMP_DIR/agent-restart"
mkdir -p "$restart_root/book/追踪"
...
printf 'book\n' > "$restart_root/.active-book"
printf '# 写作进度\n' > "$restart_root/book/追踪/上下文.md"
restart_out="$(run_from_nested "$restart_root" session-start.sh || true)"
echo "$restart_out" | grep -q 'custom agent' || fail "session-start did not confirm newly registered custom agents"
[ ! -f "$restart_root/.claude/.agents-pending-restart" ] || fail "session-start did not clear .agents-pending-restart marker"
echo "  OK TS4.2 agent restart marker"
...
stale_v14_root="$TMP_DIR/stale-v14"
mkdir -p "$stale_v14_root/.claude/agent-references/novel-assistant"
setup_git_repo "$stale_v14_root"
copy_hooks "$stale_v14_root"
cat > "$stale_v14_root/.story-deployed" <<'SENTINEL'
deployed_at: 2026-05-24T00:00:00Z
agents_version: 14
...
assert_grep 'guard-outline-before-prose\.sh' "$SETTINGS_FILE" "settings hooks must register outline-before-prose guard"
assert_grep 'Write\|Edit\|MultiEdit' "$SETTINGS_FILE" "outline-before-prose guard must run on Write/Edit/MultiEdit"
...
assert_grep '不直接改正文' "$SKILL_DIR/references/templates/agents/style-learner.md" "style-learner must not directly edit prose"
...
assert_grep 'repair_action.*Stage 3|Stage 3.*repair_action|/novel-assistant 重新拆解对标书' "$SKILL_DIR/references/templates/agents/story-explorer.md" "story-explorer must provide a v12 repair action instead of silent fallback"
assert_grep 'legacy_deconstruction: true|missing_primary_contract' "$REPO_ROOT/src/internal-skills/story-long-write/SKILL.md" "story-long-write must not silently fallback for v12 primary contract gaps"
assert_grep 'check-ai-patterns\.js' "$SKILL_FILE" "story-setup must deploy check-ai-patterns runtime script"
assert_grep '禁止先否定再肯定|禁止否定铺垫' "$SKILL_DIR/references/templates/agents/narrative-writer.md" "narrative-writer must hard-ban not-then-is flips"
