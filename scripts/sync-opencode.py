...
def replace_claude_paths(body: str) -> str:
    ...
    """
    replacements = [
        (".claude/agent-references/", ".opencode/agent-references/"),
...
def fix_path_rules_section(body: str) -> str:
    ...
    opencode path resolution. novel-assistant is the primary bundle name;
    story-setup paths are legacy fallback only.
    ...
    replacement = (
        ...
        r'1. `{项目根}/.opencode/agent-references/novel-assistant/{文件名}`\n'
        r'2. `{项目根}/.claude/agent-references/novel-assistant/{文件名}`\n'
        r'3. `{项目根}/skills/novel-assistant/references/agent-references/{文件名}`（旧项目兼容 fallback）\n'
        r'4. `{项目根}/src/internal-skills/story-setup/references/agent-references/{文件名}`（旧项目兼容 fallback）\n'
        r'\n'
        r'以上路径全部文件不存在时，才使用 Glob/Grep 全局搜索 `**/{novel-assistant,story-setup}/references/agent-references/{文件名}`。'
    )

    new_body, count = re.subn(pattern, replacement, body, flags=re.DOTALL)
    if count == 0 and "## 参考文件路径规则" in body:
...
def sync_agents():
    ...
    src_dir = ROOT / "src/internal-skills/story-setup/references/templates/agents"
    dst_dir = ROOT / "src/internal-skills/story-setup/references/opencode/agents"
...
def sync_agents_md():
    ...
    src = ROOT / "src/internal-skills/story-setup/references/templates/CLAUDE.md.tmpl"
    dst = ROOT / "src/internal-skills/story-setup/references/opencode/AGENTS.md.tmpl"
...
def write_text_lf(path: Path, content: str) -> None:
    """Write UTF-8 text with LF newlines on Python versions lacking Path.write_text(newline=...)."""
...
def main():
    ...
    print("  - src/internal-skills/story-setup/references/opencode/plugin.ts (hooks logic)")
    print("  - src/internal-skills/story-setup/references/opencode/commands/ (slash commands)")
    print("  - src/internal-skills/story-setup/references/opencode/opencode.json.patch (config fragment)")
    print("\nDone.")
