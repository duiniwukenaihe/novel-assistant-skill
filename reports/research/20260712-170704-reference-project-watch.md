...
- Checked at: `2026-07-12T17:07:04.264Z`
...
| `changed` | wen1701/FanqieRankTracker | `main` | `0f0bad08cb15` | `changed` | - | `none` | `-` | `5a0315e0aeaf` | `reference-high` | `MIT-confirmed-in-private-download-docs` | `compatible-implementation-review` | `triage_for_possible_backport` | fanqie_font_decode, ranking_scrape, book_id_extraction, anti_scraping_boundary |
...
| `changed` | yangsonhung/awesome-agent-skills | `main` | `0b9631548f26` | `changed` | - | `none` | `-` | `c3c05835bbf3` | `reference-low` | `curated-links-check-target-license` | `discovery-index-only` | `research_report_then_clean_room_triage` | skill_discovery, agent_skill_patterns |
...
| `current` | EdwardAThomson/NovelWriter | `main` | `7596f9f2e923` | `current` | - | `none` | `-` | `7596f9f2e923` | `reference-medium` | `unknown-check-before-use` | `clean-room-design-only` | `no_action` | scene_chapter_batch_review, quality_trend |
...
| `changed` | Narcooo/inkos | `master` | `7ac8d5305571` | `changed` | v1.7.0 | `untracked` | `-` | `d316c8e4fee9` | `reference-high` | `AGPL-3.0` | `clean-room-design-only` | `research_report_then_clean_room_triage` | audit_dimensions, bounded_revision, snapshot_lock, structured_delta |
...
- `wen1701/FanqieRankTracker@0f0bad08`：变化集中在日常榜单数据，没有新的稳定采集协议或代码修复，本轮 `skip/data-only`；只推进观测基线。
- `yangsonhung/awesome-agent-skills@0b963154`：继续作为发现索引，不把聚合列表当实现来源，本轮 `skip/discovery-only`。
- `Narcooo/inkos@7ac8d530` / `v1.7.0`：stale lock 恢复、路径规范化和翻译更新有参考价值；本项目已有持久会话租约、待认领状态、realpath containment 和跨平台输出，因此判定 `already-covered`。保持 AGPL clean-room 边界，不复制代码。
