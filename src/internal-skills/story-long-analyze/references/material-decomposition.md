...
| 能力/成长规则 | 名称、等级、晋升方式、能力边界、代价（文本描述） | `设定/世界观/能力与规则.md`（>=200字独立，否则并入 背景设定；按题材可命名为 `修炼与能力规则.md`、`武学与境界.md`、`系统能力与成长规则.md` 等） |
...
Stage 2 使用 chapter-extractor agent 并行处理（每章一个 agent，每批 5-8 个），并以 `批次计划.json` 的 20-40 章窗口作为可恢复边界。窗口完成即落盘摘要和 `_progress.md`，但默认继续下一窗口；批次不是用户确认点。只有接近上下文/时间边界、出现需要用户裁决的 source 冲突、或用户要求暂停时，才主动停在 `paused_at_batch_boundary`。不要把全书摘要或原文一次塞进上下文。
...
**上下文预算规则**：Stage 3-5 优先读取已落盘摘要和阶段产物，不读取全书原文；若 `_章节摘要汇总.md` 或某阶段输入超过当前上下文预算，按 `批次计划.json` 或智能分块逐块聚合，先输出中间块文件，再合并中间结果。禁止用不可执行的总耗时或风险描述吓退用户，必须给出当前批次交付和可恢复断点。
