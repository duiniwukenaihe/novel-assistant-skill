...
拆文进入 Stage 5 时加载 [deconstruction-to-author-absorption.md](deconstruction-to-author-absorption.md)。终态报告必须像作者学习笔记，而不是机械摘要：每条可借鉴技巧都要说明读者欲望、技巧机制、可迁移骨架、写作动作和禁抄替换。
...
> 状态：已完成 Stage 0-1，{已暂停 / 已继续生产级分批拆解}
...
一句话结论：基于黄金三章质量，建议{继续生产级分批拆解 / 重点拆解某部分 / 此书参考价值有限}。

## 下一步

继续生产级分批拆解将产出：逐章摘要、剧情聚合、角色档案、世界观设定、完整拆文报告。长篇按批次落盘，可从 `_progress.md` 续跑。
```
...
> P 块硬约束：每个情节点必须是独立块，`P{序号}` 必须在行首；不得把 `P2/P3` 接在上一条引用、主题标签或基调后面。每个 P 块必须包含 P 标题行、原文引用行、`主题标签... | 基调：...` 末行；P 块之间空一行。
> 末行硬约束：每个 P 块都必须有且只有一行 `主题标签... | 基调：...`。`基调：` 全角冒号不可省略（Stage 6 文风按它 grep）；`主题标签` 不加冒号；两者各取上列枚举、勿混（温馨/紧张/甜是基调、非主题）；都不贴合用「其他」。枚举权威定义见 chapter-extractor.md「基调/主题标签」节。
...
`设定/世界观/能力与规则.md`（按题材可改名为 `修炼与能力规则.md`、`武学与境界.md`、`系统能力与成长规则.md` 等）：
- 文本描述，含等级、晋升条件、能力边界、代价。长篇注意多层级成长规则
...
`设定/世界观/背景设定.md`：
- 核心规则 + 特殊设定
- 内容不足独立成文件的「能力/成长规则 / 地理 / 势力」也合并到这里
