...
**人物合同硬门**：加载 [references/character-contract.md](references/character-contract.md)。`long_startup.character_design` 与 `long_write.story_bible` 的完成回执在被状态机接受前，必须通过确定性人物合同：主角目标/软肋/缺陷/能力边界/成长里程碑，主要压力角色的利益/资源/边界/升级路径，以及关键关系债缺一不可。支持合并人物表和 `设定/角色/{角色名}.md` 一人一档，不因目录差异误伤。只读审阅可带风险继续；新增规划或正文必须先补人物。通过后自动投影压缩人物记忆，后续章节只召回当前相关人物。
...
新书进入规划阶段（Phase 1→2）时，先做一次轻量本地对标发现，**只发现、不重写消费链**——后续 Phase 2/4 的既有对标消费规则不变。

扫描范围（全部本地、只读、不联网）：
1. 项目根 `对标/` 目录及其子目录。
2. 工作区 `拆文库/{书名}/`（analyze 产出）。
3. 已登记素材库引用（如 `设定/题材定位.md` 的 `主对标书` 字段）。

发现后只返回：**1 个主对标候选 + 最多 2 个辅助候选**；每个候选展示用途（结构/情绪/节奏/文风）与不可照搬风险。

执行规则：
- 找不到本地对标时继续正常规划，**不阻断、不联网、不要求用户先拆文**。
- 用户明确说"不需要对标 / 跳过对标"时完全跳过本步，不重复询问。
- 本步只是发现和展示候选；是否采用、采用哪本由用户在 Phase 2 决定。
