 ...
 *
 * 用法：
 *   --format markdown|v0.8|both   Output format. Default: markdown.
 ...
 *   node fanqie-rank-scraper.js --channel 1 --type 2 --format v0.8 # markdown|v0.8|both
...
function assertFormat(format) {
  if (!["markdown", "v0.8", "both"].includes(format)) {
    throw new Error(`未知 --format: ${format}（可选 markdown|v0.8|both）`);
  }
}

function buildV08Artifacts(markdownFile, artifactOutdir, options) {
  if (FORMAT === "markdown") return;
  const builder = [
    path.resolve(__dirname, "../../../..", "scripts", "scan-artifact-build.js"),
    path.resolve(__dirname, "../../..", "scripts", "scan-artifact-build.js"),
  ].find((candidate) => fs.existsSync(candidate) && !candidate.includes(`${path.sep}references${path.sep}scripts${path.sep}`));
  if (!builder) throw new Error("scan-artifact-build.js not found");
  ...
  if (result.status !== 0) {
    const message = (result.stderr || result.stdout || "scan-artifact-build.js failed").trim();
    throw new Error(`scan-artifact-build.js failed: ${message}`);
  }
...
function main() {
  ...
  for (const ch of channels) {
    for (const ty of types) {
      try {
        ...
        buildV08Artifacts(filepath, path.join(OUTDIR, path.basename(filepath, ".md")), {
          platform: "fanqie",
          channel: ch === "1" ? "male" : "female",
          listName: `${channelLabel(ch)}${typeLabel(ty)}`,
          type: "long",
          captureMode: "browser",
        });
      } catch (chErr) {
        console.error(`[fanqie] ${channelLabel(ch)}${typeLabel(ty)} 采集失败，跳过: ${chErr && chErr.message ? chErr.message : chErr}`);
        if (FORMAT !== "markdown" && chErr && /scan-artifact-build\.js/.test(chErr.message || "")) {
          artifactFailures += 1;
        }
      }
    }
  ...
  if (artifactFailures > 0) {
    throw new Error(`scan-artifact-build.js failed for ${artifactFailures} output(s)`);
  }
}
