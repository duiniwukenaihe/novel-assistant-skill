...
### 规则 3：句长随场景功能变化，不追求统一短句

紧张、惊愕、决断可以短；解释、观察、关系变化和复杂动作需要完整句法。句长由场景功能决定，不存在"越短越好"。

| 场景功能 | 句法倾向 | 示例 |
|------|------|------|
| 打斗/惊变 | 短促、碎断 | 他侧身。刀光一闪。血溅在墙上。 |
| 对话 | 口语化 | "你疯了？""可能吧。" |
| 日常/观察 | 完整句、信息密度适中 | 厨房里有炒菜的声音，闻着像辣椒炒肉。 |
| 关系变化/解释 | 完整复句，允许逗号长句 | 他原以为她会拒绝，没想到她只是把钥匙放在桌上，连一句话都没问他去哪。 |

判断依据是密度，不是句数：

- 连续短句、单句成段只有在密集出现时才提示，偶尔的短句或单句成段正常。
- 连续逗号长句同样是密度问题，不是一律禁止。
- 不要为了通过检测器机械拆句，也不要机械把长句硬拼成一段。

情绪词可以使用，但必须由人物处境、动作、关系或场景证据支撑。"他很伤心"仍然是告诉；"他很伤心，把车停在路边坐了二十分钟"里的情绪词被行动赋予了证据，可以使用。禁止的是无证据的情绪标签，不是情绪词本身。
...
- 破折号密度是否失控：正文标题行以外 `——`/`—`/`--` 允许合理少量、有功能使用；`--` 和单个 `—` 需规范化；不得残留逐字破折号化；同一章或同一篇破折号密度失控是 AI味和模板停顿
...
**但删解释腔 ≠ 把读者读懵**：新名词/新设定/新道具首次出现时，仍要让读者抓到一个锚——靠角色的动作反应、对话里半句自然提及，或场景里的物理后果，一笔带出它此刻的作用或分量；既不整段讲来历原理，也别只甩一个零信息生词让读者干懵。例：「蓝晶」首次出现时，不写“这是储存记忆的装置”，但可以写她把蓝晶按上太阳穴，别人的记忆碎片炸开在眼前——功能被读者看见，全貌留作悬念。区分：锚是“角色此刻撞上的可感知后果”（留），解释是“作者跳出来讲设定来历/原理”（删）。
...
去 AI 味必须先判断对象是短篇、长篇还是未知片段。短篇优先保留商业强情绪和第一人称钩子；长篇优先保留章节连续性、人物声线和状态账本。

长篇执行时，事实保留优先于降 AI 分。不得为了降低检测器分数改章节标题、必须 beat、伏笔位置、人物状态、能力/成长规则、时间线、因果链、章节契约或交接包。若降 AI 味需要新增情节、补伏笔、移动钩子或改章节承诺，立即返回 `blocked_revision_required`，升级为回炉/修订任务。
...
- 过度使用"于是""然而""此刻" -> 删掉一半
- 所有角色说话一样"高级" -> 区分语气
- 破折号门禁 -> 正文标题行以外破折号允许合理少量、有功能使用；逐字破折号化必须回炉重写；出现逐字破折号化、`--`、单个 `—` 或破折号密度失控时，改成动作 beat、短句、换行、逗号或句号；正文标题行以外不反复堆砌 `——`
...
- 不同角色能通过对话区分吗？
- 有没有一个细节是这个场景特有的？
- 破折号门禁：正文标题行以外是否合理少量且有功能？逐字破折号化或破折号密度失控时不得声称去AI味完成，必须回炉重写或人工改写
...
- [ ] **正文破折号不过量**：正文（含叙述和对话）允许少量有功能的 `——`；`—` 和 `--` 需规范化；不得逐字破折号化、密度失控或把破折号当万能对话打断工具
...
- [ ] **情绪有现场证据**：可以写“愤怒/伤心/紧张”，但必须由当前场景的动作、关系变化或具体物件支撑，不能只贴标签或套通用身体反应
