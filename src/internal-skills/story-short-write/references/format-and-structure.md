...
| 被打断 / 拖长音 | 优先用动作打断、换行、短句或未完成动作；必要时可少量使用有功能 `——` | 禁止 `--` 和单个 `—`；`——` 不得逐字化或反复堆砌，逐字破折号化必须回炉重写 |
...
- 先用总目标字数 ÷ 计划节数得到作品级参考，再在小节大纲中按结构功能写明本节目标；不得把所有题材固定切成 800-1500 字
- 本节生产验收区间为目标的 -10% 至 +20%；低于下限或高于上限都自动回炉，不让作者确认超限正文
- 历史小节中位数只作节奏参考，不能替代当前 Brief / 小节大纲里的明确目标，也不能随正文越写越短而降低标准
- 开篇、反转、高潮、结尾只有在 Brief 事先记录明确结构理由时才允许有功能地偏长或偏短，正文仍须通过故事价值门
- 8-15 个小节只是常见规模，最终节数必须由总目标字数、题材密度与每节独立情节点共同反推
- 每小节推进一个明确的情节点
...
6. **破折号密度门禁**：正文（含叙述、对话、心理描写）允许合理少量、有功能的 `——`，禁用无功能破折号 `——`/`—`、双连字符 `--` 和省略号停顿，不得把破折号当万能对话打断工具。强打断、话语骤停、拖长音都改成动作 beat、短句、换行、逗号或句号。逐字破折号化必须回炉重写；破折号密度失控时不得机械清理，必须回炉重写或人工改写。交付前执行确定性标点收尾。
7. **禁止通篇句号化或随机标点堆砌**：标点必须跟语气/人物声线/情绪功能匹配；该质问时用问号，该爆发时少量感叹；犹豫、吞咽、未说完用动作或句长变化表达，不用无功能 `……` / 滥用 `——` 硬造停顿，也不得无功能地乱撒 `？`/`！`
...
- [ ] 正文（含对话）里的 `——`/`—`/`--` 是否已改为逗号、句号、动作 beat 或短句断开？正文标题行以外破折号是否合理少量且有功能？
