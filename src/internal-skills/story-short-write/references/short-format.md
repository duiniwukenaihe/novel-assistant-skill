...
| 优先级 | 格式 | 适用 |
|--------|------|------|
| 首选 | `“说话内容”`（中文弯引号） | 短篇默认、番茄、红果、黑岩 |
| 平台 / 项目指定 | `「说话内容」` | 知乎盐言、部分古言、用户指定 |

**番茄 / 红果 / 黑岩默认用 `“”`**；只有目标平台明确为知乎盐言、港澳台繁体、二次元弹幕体，或用户明确指定 `「」` 时才使用 `「」`。番茄短篇里密集 `「」` 会显得翻译腔 / AI 格式腔，写完后需运行 `normalize-punctuation.js --quote-mode=mainland 正文.md` 统一为 `“”`。
...
**本仓库正文不使用 `……`、单个 `—` 或 `--` 制造停顿。** 中文破折号 `——` 可在确有语义功能时少量保留；逐字破折号化、连续堆叠或超过动态密度门禁才属于阻断问题。
...
- [ ] 对话独立成行，引号风格符合平台（番茄默认 `“”`；盐言 `「」` 合法），对话标签没机械化？
