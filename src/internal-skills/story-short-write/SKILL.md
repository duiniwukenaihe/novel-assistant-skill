...
每节必须锁定目标字数。生产验收区间采用不对称容忍：最低为目标的 -10%，最高为目标的 +20%；区间内不因字数阻断，继续检查剧情功能、情绪、钩子、人物选择和承接。低于最低线或高于最高线都自动回炉当前节，不把执行质量问题交给作者确认；偏短补真实事件、冲突、选择或钩子，偏长删重复和无功能段落，承担项过载则回规划拆解。开篇、反转、高潮、结尾只有在 Brief 事先记录明确结构理由时才可作为例外；不得在正文完成后临时补理由绕过回炉。
...
**字数判定必须使用容忍策略**：优先运行 `node scripts/word-count-tolerance.js --actual <当前节中文字数> --target <本节目标字数> --unit section --json`。返回 `under_target_within_tolerance` 或 `over_target_review_pacing` 时不是失败；返回 `under_target_repair_required` 或故事质量门判定“缺剧情功能”时自动修订当前节。
**还要区分“目标”与“历史长度”**：优先使用当前 Brief 或小节大纲已确认的本节目标；旧项目当前大纲丢失逐节目标时，只能从标题与节数完全一致的已接受大纲事务恢复。全篇目标中点 ÷ 锁定节数和最近小节滚动中位数只作迁移/节奏提醒，不能成为硬性验收目标，更不能把越写越短后的中位数当成新标准。篇幅策略由统一机器门内部调用，主会话不得单独运行 `short-section-length-policy.js`。低于目标 -10% 或高于目标 +20% 时自动进入当前节回炉，不显示“保留超限正文”的菜单。正文为空或残缺到无法审阅时同样阻断。
...
如果当前节触发 `under_target_repair_required`、`over_target_repair_required` 或故事质量门失败 → 自动修当前节并重新验收，不生成下一节 Brief，不要求作者替执行缺口做决定。
...
**写完后对照 小节大纲.md 检查**：每个子事件三个维度都揉进了？本节情绪到位？伏笔/物件已植入？若低于本节目标 10% 下限或高于 20% 上限 → 自动回炉并重新验收后再写下一节。
