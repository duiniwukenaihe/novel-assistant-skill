...
const ranked = rankChineseMemory(entries, '密使', { aliases: { 密使: ['苏禾'] }, limit: 2 });
if (ranked.length !== 2) throw new Error(JSON.stringify(ranked));
if (ranked[0].entry.id !== 'fact.supporting.identity') throw new Error(JSON.stringify(ranked));
...
const { rankChineseMemory } = require(process.argv[2]);
const entries = [
  { id: 'identity', subject: '苏禾', predicate: '身份', object: '密使', aliases: ['密使'], dependencies: ['身份线索'], evidence: [{ path: '追踪/伏笔.md' }] },
  { id: 'cause', subject: '身份线索', predicate: '导致', object: '身份显现', aliases: ['身份线索'], dependencies: [], evidence: [{ path: '追踪/时间线.md' }] },
  { id: 'overlap', subject: '密使候选', predicate: '住处', object: '密使峰', aliases: [], dependencies: [], evidence: [{ path: '设定/地点.md' }] },
];
const ranked = rankChineseMemory(entries, '密使', { limit: 3 });
...
if(!rows.find(item=>item.fact_id==='fact.identity-clue.trigger').reasons.some(reason=>reason.code==='causal_dependency')) throw new Error(JSON.stringify(rows));
NODE
}
