...
下游写作消费时，短篇规则优先加载 `short-craft.md` 与 `short-deslop.md`：拆文产物提供故事核、共鸣机制和反转铺垫，正文生成与去 AI 味口径仍按短篇专用 craft/deslop 执行，不回退到长篇 anti-AI 默认流程。
...
| 4 | 反转+写作手法 | `拆文报告.md`（反转段） + `写作手法.md` | 前置反转检查 / 反转分析（铺垫 ≥2） / 写作手法 ≥5 项 / 人性共鸣学习卡 ≥3 张 |
| 5 | 人物+开头结尾 | `拆文报告.md`（人物段+首尾段） | 人物分类+功能评估 / 开头分析 / 结尾分析 / 首尾呼应 |
| 6 | 综合评估 | `拆文报告.md`（综合段） + `_meta.json`（写 structure_counts） | 五维评分 / 爆点性 / 话题性 / 共鸣 ≥3 层 / 共鸣机制回收 ≥3 条 / 可复用结构 ≥3 条 / 节奏速报 |
...
用户「拆解完成，可调用 `/novel-assistant 写短篇` 写下一篇」。
...
| `写作手法.md` | 手法库 + 共鸣学习卡 | POV / 对话 / 时间 / 信息控制 等具体手法 + 原文示例 + 人性共鸣学习卡。新篇里复用机制，不复用桥段 |
| `原文/` | 语感源 | 抄对话调子、节奏、画面感、打脸张力。**不抄具体情节**，抄写法。 |

### 写作流程建议

1. 看 `_meta.json.genre_detected` 和 `structure_counts.reversal_type` 选骨架。
2. 读 `拆文报告.md` 的「核心手法」「共鸣分析」「共鸣机制回收」「可复用结构」段，决定要保留 / 调整哪些。
3. 读 `情节节点.md` 把节奏锚点抄到新故事的字数位置上。
4. 写场景时翻 `写作手法.md` 的「人性共鸣学习卡」+ `原文/`，参考具体写法。只迁移人物软肋、关系压力、生活物件、选择代价和情绪债的机制，不能换名照搬原桥段。
