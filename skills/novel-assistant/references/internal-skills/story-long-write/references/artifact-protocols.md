...
## 对标结构坐标（如有对标书）
| 对标一级结构 | 本卷章节范围 | 关键点(1/4/中点/3/4) | 本书替换事件 | 章节定位 |
|--------------|-------------|----------------------|--------------|----------|
| {对标功能/情绪链} | {章X-Y} | {逆转/转折/激励} | {完全换素材后的事件} | {高压/推进/关系/低压/信息整理} |
...
- 核心事件：{一句话}
- 字数目标：{X} 字
- 章节定位：{高压章/普通推进章/修炼试错章/关系回收章/低压生活章/信息整理章；可复合，如“高压+关系”}
- 目标情绪：{本章交付什么情绪}
- 章首钩子：{类型} — {具体内容}
- 爽点：{本章爽点 / 无显性爽点但承担的功能}

#### 质量触发
- 激活标签：{人物线、悬念推进、能力切换等按本章风险选择的标签}
- 激活原因：{本章为何需要该维度，例如“本章改变核心关系，并把旧问题推进到新证据”}

#### 呈现与连续性
- 可见证据：{必须出现在场上的动作、物件或反应}
- 前置承接：{继承的状态、承诺或压力}
- 本章变化：{情绪、关系、信息、目标或力量至少一项发生的可验证变化}
- 后续债务：{本章留下、升级或准备回收的债务}
...
**状态含义**：`未埋` = 已规划但尚未埋设；`已埋` = 已埋设且仍开放，是长篇正常状态；`已回收` = 已关闭；`已过期` = 已错过预计回收窗口，需要 `/novel-assistant 审查` 或显式修复。SessionStart hook 不应仅因 `未埋`、`已埋` 或 `已回收` 报警。日更流程只处理本轮新增/推进/回收的伏笔，全量伏笔审计必须由内部模块 `story-review` 或用户明确要求触发。
