...
可以使用情绪词，但必须由人物处境、动作、关系变化或场景物件提供证据。禁止的是脱离现场的情绪标签，不是情绪词本身。身体反应只是证据之一，不得机械替换成“攥拳、发抖、指甲掐进掌心”。

### 证据化操作

扫描正文中的情绪词：有现场证据就保留；只有抽象结论时，补入当前场景独有的动作、关系或物件，必要时再删去标签。
...
| 情绪词缺少场景证据？ | 情绪标签悬空 | 保留必要情绪词，并补当前场景独有的动作、关系或物件证据 |
...
标点不是装饰，而是语气、权力关系和情绪节拍的可见形态。写正文时先判断“这句话在场景里做什么”，再决定用句号、逗号、问号、感叹号、冒号或换行/动作 beat；不再用省略号和破折号机械制造停顿；少量有功能的中文破折号可保留。
...
| 被打断 / 拖长音 | 动作打断、换行、短句、未完成动作 | 用“他抬手截住她的话。”这类动作切断 | 优先不用符号硬造停顿；确有强打断功能时可少量用 `——`，禁用 `—`/`--` |
...
3. 正文（含对话）里的 `……`、`—`、`--` 改成句号、逗号、短句、换行或动作断句；`——` 仅在承担明确语义功能时少量保留，密度异常或逐字化必须修订。知乎盐言 `「」` 只是引号风格，不影响问号、感叹号的合法使用。

---

## 细纲责任与正文表达的边界

细纲传递的是故事责任，不是正文的形状。正文必须自主组织场景、动作、对白、感官、停顿和段落。

**细纲负责传递**：事件责任、人物选择、代价、关系变化、信息揭示、承接责任。

**正文负责组织**：场景入口、动作序列、对白节奏、感官细节、停顿与留白、段落呼吸。

**禁止**：禁止按细纲字段顺序逐条翻译成正文（"目标是…阻力是…反转是…"）；不得把"目标/阻力/爽点/下节钩子"等字段名写进叙述或角色内心；禁止章尾总结本节意味着什么（"这一章意味着他终于长大了""总结一下，这一节…"）。
- 禁止叙述者做工程化预告（"下一节他将面对真正的敌人""下一章他会…"）。场景内已经发生的动作、对白或证据可以形成自然钩子，例如角色当场发出律师函、收到新证据或作出不可撤回的选择。

**正确做法**：通过动作、冲突、对白和结果兑现同一细纲责任，但不出现字段名和说明腔。章尾可以自然留白，不要求每节强行悬念句；如果一个动作或对白已经完成了本节责任，不需要再用叙述句复盘。
