...
同人 / 原创 / 历史衍生 / 既有世界观续写对应不同世界来源模式，决定命名护栏是否启用：

| 模式 | 命名护栏 |
|------|---------|
| `original` | 不强制启用 |
| `fanfic` | 启用：新增命名符合原作规律 |
| `historical_derivative` | 启用：新增命名符合时代/地域规律 |
| `imported_existing_world` | 启用：新增命名符合既有世界规律 |

启用模式下，护栏为 advisory：发现命名偏离时给 advisory 和替代方案，不硬阻断；不因命名护栏阻止用户明确要求的合理改编；不预先硬编码"常见中文名"名单，规律来自原作/时代样例。
