...
设计角色时，先判断本书的世界来源模式，决定命名护栏是否启用：

| 模式 | 含义 | 命名护栏 |
|------|------|---------|
| `original` | 完全原创世界观 | 不强制启用；按本书设定自洽即可 |
| `fanfic` | 同人作品，借用已有 IP 世界观 | 启用：新增人名必须符合原作命名规律 |
| `historical_derivative` | 历史衍生（穿越/架空历史/历史同人） | 启用：新增人名、地名、组织名必须符合时代与地域命名规律 |
| `imported_existing_world` | 既有世界观续写、导入后续写 | 启用：新增角色、组织、地名必须符合既有世界命名规律 |

**启用规则**（`fanfic` / `historical_derivative` / `imported_existing_world`）：

- 新增人名、地名、组织名、技术名必须符合原作 / 时代 / 地域的命名规律。
- 护栏为 advisory：发现命名偏离规律时给出 advisory 和替代方案，不硬阻断；不因为命名护栏阻止用户明确要求的原创改编或合理改编。
- 不把"常见中文名"做成硬编码名单——规律来自原作/时代样例，不是预先枚举的中文名池。
- 冲突时：先标注 advisory + 替代方案，让用户裁决是否保留。

`original` 模式（普通原创）不强制启用，普通原创短篇不适用。
