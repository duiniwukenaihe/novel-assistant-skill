...
只接受 `workflow_type=download_import` 的阶段包。必须锁定来源文件、目标项目、导入模式、覆盖边界、`workflow_id`、结果包路径和 Runner 的 `memory_context`；不得扫描父目录猜目标书。

### Outputs To story-workflow

每阶段回传标准 result packet，列明导入文件、解析失败项、目录映射、身份冲突、验证结果、断点和 `memory_updates`。导入只生成候选项目事实；未经校验不得覆盖已有正文、大纲、细纲或追踪资产。

### Memory Policy

`download_import` 使用 `optional` 记忆策略。导入新项目时通常只有任务上下文；合并到已有项目时才召回范围相关 canon。解析得到的人物、伏笔和规则先进入建议与冲突校验，不能直接污染已确认记忆。
...
深度分析阶段调用现成的内部拆解管道，不重新发明：长篇进入 `story-long-analyze` 的完整拆解管道，短篇进入 `story-short-analyze` 的拆解管道。拆解方法论与输出模板由对应 analyze 模块自带，story-import 不执行拆解方法论、不维护这些文件。

### 原则 3：导入是反向建立连续性资产，不是静默重写

story-import 的职责是反向建立连续性资产：章节索引、角色状态、时间线、伏笔、设定、卷/章结构、拆文库对标资产和可续写工程。默认**不擅自改写原正文**，不因为格式、AI味、节奏或设定问题自动重写章节。

- 可以做：切章、归档原文、建立 `追踪/`、补结构索引、标注缺口、生成后续续写所需的上下文。
- 必须做：保留原文来源与导入前版本，标记残章、缺章、重复章、章节名冲突和无法确认的连续性 gap。
- 不能做：静默改章节名、静默重排正文、静默补写缺失章节、把拆文结论当作用户原文事实。
- 若用户要求“顺便修一下”：升级为 `story-long-write` 或 `story-short-write` 的回炉/精修流程，先确认影响范围和版本快照。

## 全局可见长回复污染门禁

导入诊断、结构迁移方案、缺口报告、连续性资产总结和导入完成报告超过 800 中文字符时，不得直接输出长报告。先写入 `追踪/输出门禁/.visible_reply_draft_{YYYYMMDD_HHMMSS}.md`（导入项目可用当前工作目录），运行 `node scripts/output-pollution-check.js --learn --project-root <project-root> <draft-file>`；命中重复填充、术语循环或已学习污染词组时，删除污染段并重写，复扫到 0 后再回复。若污染已经开始输出，立即停止并落盘 `paused_after_output_pollution`。
...
- 只要分析 / 拆文库 → 直接进入 `story-long-analyze`（短篇 `story-short-analyze`），到拆文库为止，不进 Phase 3 迁移；对用户提示统一写 `/novel-assistant 拆长篇` 或 `/novel-assistant 拆短篇`。
...
> 「检测到当前项目尚未部署写作基础设施。建议先运行 `/novel-assistant 准备写书` 再回来导入，否则深度分析阶段无法使用并行 chapter-extractor agent。」

给用户两个选择：

1. **先去 setup**：暂停导入，运行 `/novel-assistant 准备写书`，部署完成后重新触发 `/novel-assistant 导入小说`；
...
story-long-analyze 在 Stage 0+1（黄金三章）后会**自动停靠**并用普通文本确认询问是否继续全量拆解（对应 story-long-analyze 的「Stage 1 停靠点」）。但导入场景需要 Stage 2-6 的全套产物（逐章摘要 / 聚合分析 / `剧情/节奏.md` / `剧情/情绪模块.md` / 设定关系 / 汇总报告 / 文风），缺一不可——否则 Phase 3 迁移会拿到半成品。
...
> 管道详细说明见内部模块 story-long-analyze（用户入口：`/novel-assistant 拆长篇`），此处仅列概要。
...
> 管道详细说明见内部模块 story-short-analyze（用户入口：`/novel-assistant 拆短篇`），此处仅列概要。
...
导入小说通常同时包含切章、拆文、结构迁移和写作工程生成。出现 API error、timeout、会话压缩、迁移中断或文件冲突时，按长任务自愈处理，不从头覆盖已完成产物。

1. **文件系统是权威**：优先读取 `拆文库/{书名}/_progress.md`、`章节切片索引.jsonl`、`批次计划.json`、导入进度文件、章节映射表、已生成的 `正文/`、`大纲/`、`设定/` 和 `追踪/schema/`。聊天记忆只作参考。
2. **分阶段恢复**：拆文未完成时回到 story-long-analyze / story-short-analyze 的第一缺口；拆文完成但结构迁移未完成时，从章节映射表或已生成目录定位第一未迁移章节；schema 已生成但校验失败时，只重跑 schema build/validate，不重拆原文。
3. **自动修复类**：缺少章节索引但原文仍在时先重建；卷内目录缺失但正文文件存在时按章节资产重建目录；`_progress.md` 与实际文件冲突时，以实际文件和最近修改时间为准修正进度。
4. **外部阻断类**：源文件丢失、多源版本冲突、quota / Token Plan / usage limit 不无限重试。写清断点、冲突文件和下一步需要的用户裁定。
5. **覆盖保护**：已有正文、章名、细纲和设定默认不覆盖；需要重建时先生成版本快照或冲突清单，再执行迁移。
...
| 拆文库形态 | 检测方法 | 处理 |
|-----------|---------|------|
| **已按主题拆分** | `拆文库/{书名}/设定/世界观/` 子目录存在 + 至少含 `背景设定.md`、`能力与规则.md`、`修炼与能力规则.md`、`武学与境界.md` 或 legacy `力量体系.md` 之一 | **pass-through**：原样同步 `设定/世界观/*.md` 和 `设定/势力/*.md` 到项目，无需 re-split；legacy 名称只保留原样，不作为新项目默认名 |
...
| 拆文库内容 | 项目文件 | 拆分规则 |
|-----------|---------|---------|
| 能力/成长规则 | 设定/世界观/能力与规则.md（按题材可命名为 `修炼与能力规则.md`、`武学与境界.md`、`系统能力与成长规则.md` 等） | 独立文件；不要把非修真项目机械命名为 `力量体系.md` |
...
- v12 新契约拆文库缺 `剧情/节奏.md` 或 `剧情/情绪模块.md` → **导入停下并给修复动作**：提示用 `/novel-assistant 重新拆解对标书` 跑 Stage 3+ 或手动补齐对应文件；导入报告写 `module_or_rhythm_required_missing`，不得继续生成看似完整的对标视图。
...
- 拆文库没有文风文件（analyze 未跑 Stage 6）→ 导入报告提示用户用 `/novel-assistant 重新拆解对标书` 后再同步；日更前文风缺失会被 fail-fast 拦截
...
## 下一步操作
- 运行 `/novel-assistant 审查 lean` 审查导入结果
- 运行 `/novel-assistant 日更` 开始续写
```
...
## 下一步操作
- 运行 `/novel-assistant 写短篇` Phase 3 开始续写
```
...
| 章节格式识别 | 由 story-long-analyze 拆解管道（用户入口：`/novel-assistant 拆长篇`）的阶段 1 负责 |

### Phase 2：深度分析

| 场景 | 加载文件 / 相关 skill |
|------|---------|
| 长篇深度分析（方法论、质量检查、输出模板均自带） | 进入 `story-long-analyze` 内部管道（用户入口：`/novel-assistant 拆长篇`） |
| 短篇深度分析（方法论、质量检查、输出模板均自带） | 进入 `story-short-analyze` 内部管道（用户入口：`/novel-assistant 拆短篇`） |
...
| 环境部署 | 部署模板由内部模块 `story-setup` 提供，story-import 不负责部署；用户入口为 `/novel-assistant 准备写书` |
...
| 时机 | 跳转到 | 命令 |
|---|---|---|
| 导入完想继续写（长篇） | story-long-write | `/novel-assistant 日更` |
| 导入完想继续写（短篇） | story-short-write | `/novel-assistant 写短篇` |
| 导入完想审查质量 | story-review | `/novel-assistant 审查` |
| 想深入分析对标（长篇） | story-long-analyze | `/novel-assistant 拆长篇` |
| 想深入分析对标（短篇） | story-short-analyze | `/novel-assistant 拆短篇` |
| 从零开新书（长篇） | story-long-write | `/novel-assistant 开书` |
| 从零开新书（短篇） | story-short-write | `/novel-assistant 写短篇` |
| 项目未部署环境 | story-setup | `/novel-assistant 准备写书` |
