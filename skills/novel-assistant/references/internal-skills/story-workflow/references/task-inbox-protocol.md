...
旧项目首次接入时若同时缺**写入策略**（write policy）与**任务权威**（task authority），必须按固定顺序恢复：先完成写入策略的 preview → confirm → apply，再启动旧任务权威恢复的 preview → confirm → apply。每一步只执行该步状态机返回的精确命令，不得跨步骤串联、不得提前调用后续脚本；写入策略 apply 必须先收到成功回执，才进入旧任务权威恢复。旧任务权威恢复 apply 完成后必须恢复原始业务意图，把可见候选重新交回用户选下一步，不得自动拼接未授权命令或重复缺失步骤。
