...
- 章节原文文本
- 章节字数（近似值，用于调节情节点密度）
- `OUTPUT_PROFILE`（可选）：`fast_source_grounded` 或 `deep`

## 输出档位

默认使用 `deep`。当 prompt 包含 `OUTPUT_PROFILE: fast_source_grounded` 时，必须切换为快速源锚模式，用于 200+ 章长篇的全书覆盖。

### fast_source_grounded（快速源锚模式）

- 概要：80-180 字，保持因果链，不写主观评价。
- 关键事件：3-5 条。
- 关键信息与扩写技法：1-3 条，抓本章最值得学习的扩写动作，不铺满表格。
- 出场人物：只列 major/supporting；纯龙套、仅一次提及、无剧情作用的人物不列。
- 情节点：6-12 个，只保留改变剧情、设定、关系、钩子、爆点或回收的关键节点。
- 每个情节点仍必须有一条连续原文锚点，仍必须保留 `主题标签... | 基调：...` 末行。
- 不允许因为快拆而串书、补全、推断或省略 source-grounding。

### deep（深拆模式）

用于黄金三章、卷首卷尾、爆点/转折/回收章、用户点名章、质量失败重试章。按下方「情节点密度」执行 10-40 个情节点。
...
你只能使用本次 prompt 里的「章节原文」。禁止凭记忆、常识、其他作品、前后章节猜测来补全。

- 出场人物表里的每个角色名、别名，以及每个情节点 `涉及{}` 里的具体人物，都必须能在本章原文中找到。
- 每个情节点引用必须是本章原文中的连续片段，不得改写、拼接、借用其他章节或其他作品。
- 原文未明确的人名、地点、组织、物品，写「原文未明确」或留空；不得为了完整而编。
- 如果你发现自己输出了 prompt 原文里没有出现的名字，必须删除该项并重写相关情节点。
...
P{序号+1} **{事件概括}**：类型{转折点/信息揭示/冲突/解决/铺垫/行动/对话/状态变化} | 涉及{全名，多人逗号分隔；纯环境铺垫无具体人物时本项留空} | 地点{如明确} | 物品{如涉及} | 时间{如明确}

{≤400字原文直接引用，单独成段，不加“原文引用：”标签}

主题标签{爱情/亲情/友情/权力/金钱/成长/复仇/悬念/搞笑/热血/日常/其他} | 基调：{紧张/轻松/悲伤/热血/爽/甜/温馨/恐怖/压抑/其他}

> P 块硬约束：每个情节点必须是独立块，`P{序号}` 必须在行首；不得把 `P2/P3` 接在上一条引用、主题标签或基调后面。每个 P 块必须包含 3 行：P 标题行、原文引用行、`主题标签... | 基调：...` 末行；P 块之间空一行。
> 末行格式硬约束：每个 P 块都必须有且只有一行 `主题标签... | 基调：...`。`基调` 用全角冒号 `基调：`，不可省略或换半角；`主题标签` 后不加冒号。主题标签只能取上列 12 种、基调只能取上列 10 种——“温馨/紧张/甜”等是基调值，禁止填进主题标签；都不贴合时用「其他」，勿硬塞近义项。
...
若 prompt 包含 `OUTPUT_PROFILE: fast_source_grounded`，本节只作为上限参考，实际输出 6-12 个关键情节点；不要为了凑 10 个而把同一场景切碎。若 prompt 未指定或指定 `deep`，执行下列密度规则。
...
3. 情节点是否严格按时序排列
4. 情节点数量是否符合当前 `OUTPUT_PROFILE`：`fast_source_grounded` 为 6-12 个关键情节点；`deep` 为 10-40 个动态情节点（由字数决定，不是固定值；硬下限 10）
...
12. 所有角色名/别名/涉及人物是否都出现在本章原文中，所有情节点引用是否都是本章原文连续片段；任何 `unknown_entities`、`quote_not_in_source`、`quote_missing` 都会触发重试
