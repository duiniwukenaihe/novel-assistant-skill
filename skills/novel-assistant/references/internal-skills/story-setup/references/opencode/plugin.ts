...
function proseBlockReason(root: string, abs: string): string | null {
  ...
  if (base === "正文.md") {
    ...
    if (!fs.existsSync(path.join(bookDir, "小节大纲.md"))) {
      return `⛔ 写正文被拦截：${path.relative(root, abs) || abs} 缺少同目录 小节大纲.md。先用 /novel-assistant 完成短篇小节大纲再写正文。`
    }
    return null
  }
  ...
  // 长篇分章正文：
  // - legacy: {书}/正文/第N章*.md
  // - volume-local: {书}/正文/第1卷/第N章*.md
  const dir = path.dirname(abs)
  const grandParent = path.basename(path.dirname(dir))
  const inLegacyProseDir = parent === "正文"
  const inVolumeProseDir = grandParent === "正文"
  if (!inLegacyProseDir && !inVolumeProseDir) return null
  if (!/^第.*章.*\.md$/.test(base)) return null
  if (fs.existsSync(abs)) return null // 已存在 → 续写/改稿放行
  const m = base.match(/^第0*(\d+)章/)
  if (!m) return null
  const num = m[1]
  const volumeName = inVolumeProseDir ? parent : ""
  const bookDir = inVolumeProseDir ? path.dirname(path.dirname(path.dirname(abs))) : path.dirname(path.dirname(abs))
  // story-import 迁移：已有 拆文库/{书名}/ 分析源时放行（细纲由章节摘要反推、晚于正文迁移）
  if (fs.existsSync(path.join(root, "拆文库", path.basename(bookDir)))) return null
  // 容忍补零差异与标题后缀：按整数章号匹配 大纲/细纲_第*章*.md
  const outlineDir = volumeName ? path.join(bookDir, "大纲", volumeName) : path.join(bookDir, "大纲")
  let found = false
  try {
  ...
  if (!found) {
    return `⛔ 写正文被拦截：第 ${num} 章缺少细纲（${path.relative(root, outlineDir)}/细纲_第${num}章.md）。先用 /novel-assistant 补建本章细纲再写正文。`
  }
  return null
}
