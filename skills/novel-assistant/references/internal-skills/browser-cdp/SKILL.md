...
browser-cdp 是数据采集工具，只负责打开页面、复用登录态、提取可见页面数据、截图或导出结构化原始材料。它不做写作判断，不决定选题，不改正文/大纲/细纲，不替代扫榜、拆文、审阅或写作模块。抓到的数据必须交给 `story-long-scan`、`story-short-scan`、`story-long-analyze` 或其他上层模块解释。
...
- `CDP_STATUS=needs-setup` 且 `CHROME_RUNNING=no` → 安全启动：
  ...
  ```
- `CDP_STATUS=needs-setup` 且 `CHROME_RUNNING=yes` → **先用普通文本确认**：告知会杀掉 N 个 Chrome 进程、可能丢失未保存工作；用户同意后再带 `--yes` 启动；用户拒绝则放弃这次自动化。不得调用外部交互工具。
...
| 问题 | 解决方案 |
|------|----------|
| `NEEDS_CONSENT` + 退出码 3 | 用普通文本确认询问用户是否允许杀掉 Chrome，同意后加 `--yes` 重跑 |
