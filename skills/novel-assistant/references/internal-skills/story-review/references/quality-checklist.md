...
- [ ] 破折号门禁：正文标题行以外允许少量有明确插入、转折或强打断功能的 `——`；`—`/`--` 需规范化。逐字破折号化、连续堆叠或密度失控才是阻断项，应回炉为动作、短句、逗号或句号，而不是机械清零
...
> 破折号门禁统一补充：正文标题行以外允许少量有功能的 `——`，`—`/`--` 需规范化；逐字破折号化、连续堆叠或密度失控必须回炉重写，不得机械删除后宣布完成。

> 细纲责任与正文表达边界补充：正文不得逐条翻译细纲字段（目标/阻力/反转/爽点/下节钩子），不得出现字段名和说明腔；禁止章尾总结本节意味着什么，禁止叙述者做工程化预告。正文通过动作、冲突、对白和结果兑现细纲责任；场景内已经发生的动作、对白或证据可以形成自然钩子，章尾也可以自然留白。
