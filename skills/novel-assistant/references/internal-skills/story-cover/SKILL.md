...
只接受 `workflow_type=cover` 的阶段包，读取书名、作者、平台规格、视觉方向、目标文件、覆盖策略、确认 token、`workflow_id` 和 Runner 的 `memory_context`。

### Outputs To story-workflow

每阶段回传标准 result packet，包含提示词/图片产物、实际写入路径、尺寸与文字可读性证据、确认绑定、验证结果和断点。模块不能自行宣布整个 workflow 完成。

### Memory Policy

`cover` 使用 `optional` 只读策略，只消费题材定位和已确认视觉偏好，不产生叙事 `memory_updates`，也不得反向修改人物、伏笔、大纲或正文。
...
story-cover 不参与叙事连续性治理，只消费书名、作者名、题材、视觉关键词、平台尺寸和用户提供的设定摘要。封面生成不能反向修改本书大纲、人物关系、伏笔、正文或章节承诺；如果封面构想暴露出题材定位不清，只能提示用户回到 `story-long-write` / `story-short-write` 做定位确认。

## Workflow 确认上下文硬门禁

任何生成或覆盖都只能由 `story-workflow` 的 `cover` workflow 调用。进入 Step 3 前必须读取当前任务的 `stage_execution`，并同时满足：

- `stage_execution.status=running`，`stage_id` 与 `step_id` 都是 `generate_cover_execute`；
- `stage_execution.confirmation_context.status=confirmed`，其中 `confirmation_token` 与 `stage_execution.confirmation_token`、持久化 `last_selection.confirmation_token` 完全一致；
- `confirmation_context.expires_at` 尚未过期，workflow、stage、step、selection 和可见候选哈希仍与当前任务一致；
- `confirmation_context.operation=generate` 时只允许生成新封面；`operation=overwrite` 时只允许覆盖确认上下文中指定的现有目标文件。

生成新封面和覆盖现有封面是两种独立确认，token 不得跨阶段、跨目标或跨操作复用。没有来自 workflow 的 `confirmation_context` / `confirmation_token`、token 过期、目标不一致，或用户裸调用本模块时，必须返回 `blocked_confirmation_required`，停靠并询问用户；此时不得调用 `images/generations`、`images/edits` 或写入任何封面文件。

宿主把已验证字段投影为 `WORKFLOW_CONFIRMATION_CONTEXT`（完整 JSON）和 `WORKFLOW_CONFIRMATION_TOKEN` 后，Step 3 的两种 API 调用都先执行以下门禁；失败立即退出，不得继续请求图像 API：

```bash
: "${WORKFLOW_CONFIRMATION_CONTEXT:?blocked_confirmation_required: 缺少 workflow confirmation_context}"
: "${WORKFLOW_CONFIRMATION_TOKEN:?blocked_confirmation_required: 缺少 workflow confirmation_token}"
jq -e --arg token "$WORKFLOW_CONFIRMATION_TOKEN" '
  .status == "confirmed" and
  .stage_id == "generate_cover_execute" and
  .step_id == "generate_cover_execute" and
  .confirmation_token == $token and
  (.operation == "generate" or .operation == "overwrite") and
  ((.expires_at | fromdateiso8601) > now)
' <<<"$WORKFLOW_CONFIRMATION_CONTEXT" >/dev/null || {
  echo "blocked_confirmation_required: workflow 确认缺失、过期或不匹配" >&2
  exit 2
}
```
...
封面题材分析、视觉方案、提示词解释和生成结果总结超过 800 中文字符时，不得直接输出长报告。先写入 `追踪/输出门禁/.visible_reply_draft_{YYYYMMDD_HHMMSS}.md`（没有书籍项目时用当前工作目录），运行 `node scripts/output-pollution-check.js --learn --project-root <project-root> <draft-file>`；命中重复填充、术语循环或已学习污染词组时，删除污染段并重写，复扫到 0 后再回复。若污染已经开始输出，立即停止并落盘 `paused_after_output_pollution`。
...
| `GPT_IMAGE_SIZE` | | `1024x1536` | 目标比例提示（番茄 3:4 → `768x1024`，默认 2:3 → `1024x1536`）。官方 gpt-image-2 支持 16 倍数尺寸，但很多中转代理会忽略 size；平台尺寸由 Step 3.5 兜底 |
| `UPLOAD_SIZE` | | — | 平台固定上传像素（番茄 `600x800`）；设置后 Step 3.5 居中裁剪+缩放出上传版 |
...
> **书名和作者名（笔名）是封面必需信息**：书名或作者名（笔名）缺失时，必须先向用户补问，不得编造、留空或用“佚名”占位。

**按目标平台定封面比例和上传尺寸**。番茄小说上传规格常用 600×800，是 3:4；若直接用 2:3 原图，平台二次裁切可能切掉书名或笔名。

| 平台 | 上传尺寸 | 比例 | 建议 `GPT_IMAGE_SIZE` |
|:-----|:--------|:-----|:-------------------|
| 番茄小说 | 600×800 | 3:4 | `768x1024` |
| 其他平台（默认竖版） | 按平台规格 | 2:3 | `1024x1536` |

`GPT_IMAGE_SIZE` 只作为生成比例提示；如果平台有固定上传像素，再设置 `UPLOAD_SIZE`（番茄 `600x800`）。平台尺寸最终由 Step 3.5 居中裁剪+缩放保证，不依赖代理是否尊重 size。平台与题材风格见 [references/cover-styles.md](references/cover-styles.md)。
...
[题材风格标签]. [人物描述]. [背景描述].
[色彩指令]. [光效指令].
Professional book cover, high detail digital painting, portrait [平台比例：番茄=3:4，默认=2:3] ratio, keep title and author name inside the central safe area away from edges (inner about 85%), no watermark
```
...
设置 `UPLOAD_SIZE` 后，把原图**居中裁剪+缩放**成平台精确像素，不拉伸变形，避免平台再次裁掉书名或笔名。原图保留，另存 `_上传` 版，例如 `封面_v${i}_上传.png`。
...
if [ -n "$TARGET" ] && [ -f "$SRC" ]; then
  ...
  if command -v magick >/dev/null 2>&1; then
    ...
    convert "$SRC" -resize "${W}x${H}^" -gravity center -extent "${W}x${H}" "$UP"
  elif command -v sips >/dev/null 2>&1; then
    cp "$SRC" "$UP"
    sw=$(sips -g pixelWidth "$UP" | awk '/pixelWidth/{print $NF}')
    sh=$(sips -g pixelHeight "$UP" | awk '/pixelHeight/{print $NF}')
    if [ $((sw*H)) -ge $((sh*W)) ]; then
      sips --resampleHeight "$H" "$UP" >/dev/null
    else
      sips --resampleWidth "$W" "$UP" >/dev/null
    fi
    sips -c "$H" "$W" "$UP" >/dev/null
  else
    echo "无 magick/convert/sips，跳过；请手动把 $SRC 居中裁剪+缩放到 $TARGET 再上传" >&2
  fi
  [ -f "$UP" ] && file "$UP"
fi
```

> 书名和笔名已在提示词里要求留中心安全区，居中裁剪不会切到。
...
| 构图合理 | 主体突出，文字不遮挡核心画面 |
| 平台适配 | 符合目标平台的封面风格调性 |
| 平台尺寸 | 上传版比例与平台一致；缩放到上传尺寸后书名、笔名完整可见、未被裁切 |
